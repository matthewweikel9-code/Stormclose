...
Imagery Date: ${result.imageryDate}
Data Quality: ${result.dataQuality}

ROOF MEASUREMENTS
-----------------
Total Area: ${result.measurements.totalAreaSqFt.toLocaleString()} sq ft
Roofing Squares: ${result.measurements.totalSquares}
Ground Area: ${result.measurements.groundAreaSqFt.toLocaleString()} sq ft
Number of Facets: ${result.measurements.facetCount}
Average Pitch: ${result.measurements.avgPitchRatio} (${result.measurements.avgPitchDegrees}°)

ROOF SEGMENTS
-------------
${result.segments.map(seg => 
`Facet ${seg.id} - ${seg.direction}
  Area: ${seg.areaSqFt.toLocaleString()} sq ft
  Pitch: ${seg.pitchRatio} (${seg.pitchDegrees}°)
  Azimuth: ${seg.azimuthDegrees}°
`).join('\n')}

MATERIAL ESTIMATES
------------------
Shingle Bundles (3-tab): ${result.estimates.shingleBundles}
Underlayment Rolls: ${result.estimates.underlaymentRolls}
Ridge Cap Bundles: ${result.estimates.ridgeCapBundles}
Drip Edge (linear ft): ${result.estimates.dripEdgeFeet}

COST ESTIMATE
-------------
Estimated Range: ${formatCurrency(result.estimates.costRange.low)} - ${formatCurrency(result.estimates.costRange.high)}

* Estimates include materials and labor. Actual costs may vary.

---
Report generated by StormClose AI
			`.trim();
			...
			a.href = url;
			a.download = `roof-report-${result.address.replace(/[^a-z0-9]/gi, '-').toLowerCase()}.txt`;
	...
	// Create Estimate (generates a formal estimate document)
	const handleCreateEstimate = () => {
		...
		try {
			const avgCost = (result.estimates.costRange.low + result.estimates.costRange.high) / 2;
...
Estimate #: EST-${Date.now().toString().slice(-8)}
...
- Installation of new shingles (${result.measurements.totalSquares} squares)
...
Total Roof Area:        ${result.measurements.totalAreaSqFt.toLocaleString()} sq ft
Roofing Squares:        ${result.measurements.totalSquares}
Number of Facets:       ${result.measurements.facetCount}
Average Pitch:          ${result.measurements.avgPitchRatio}
...
-----------                                    --------        
Architectural Shingles (bundles)               ${result.estimates.shingleBundles}
Synthetic Underlayment (rolls)                 ${result.estimates.underlaymentRolls}
Ridge Cap Shingles (bundles)                   ${result.estimates.ridgeCapBundles}
Drip Edge (linear feet)                        ${result.estimates.dripEdgeFeet}
Ice & Water Shield (2 rolls assumed)           2
Roofing Nails (boxes)                          ${Math.ceil(result.measurements.totalSquares / 3)}
Pipe Boots/Flashings                           As needed
...
Materials:                                     ${formatCurrency(materialCost)}
                                              ---------------
SUBTOTAL:                                      ${formatCurrency(avgCost)}
Overhead & Profit (10%):                       ${formatCurrency(avgCost * 0.1)}
                                              ===============
TOTAL ESTIMATE:                                ${formatCurrency(avgCost * 1.1)}

--------------------------------------------------------------------------------
                          ESTIMATE RANGE
--------------------------------------------------------------------------------

Low Estimate:     ${formatCurrency(result.estimates.costRange.low)}
High Estimate:    ${formatCurrency(result.estimates.costRange.high)}
...
- Warranty: Manufacturer warranty on materials, 5-year workmanship warranty
- Permit fees not included (if required)

================================================================================
                    Generated by StormClose AI
================================================================================
			`.trim();
			...
			const a = document.createElement('a');
			a.href = url;
			a.download = `estimate-${result.address.replace(/[^a-z0-9]/gi, '-').toLowerCase()}.txt`;
			document.body.appendChild(a);
			a.click();
			document.body.removeChild(a);
			URL.revokeObjectURL(url);
	...
	// Navigate to Supplement Generator with pre-filled data
	const handleGenerateSupplement = () => {
		if (!result) return;
		
		// Store measurement data in sessionStorage for the supplements page
		const supplementData = {
			address: result.address,
			roofSquares: result.measurements.totalSquares,
			roofArea: result.measurements.totalAreaSqFt,
			pitch: result.measurements.avgPitchRatio,
			facetCount: result.measurements.facetCount,
			estimateLow: result.estimates.costRange.low,
			estimateHigh: result.estimates.costRange.high,
			materials: {
				shingleBundles: result.estimates.shingleBundles,
				underlaymentRolls: result.estimates.underlaymentRolls,
				ridgeCapBundles: result.estimates.ridgeCapBundles,
				dripEdgeFeet: result.estimates.dripEdgeFeet,
			},
			segments: result.segments,
		};
		
		sessionStorage.setItem('roofMeasurementData', JSON.stringify(supplementData));
			...
			{result && (
				...
				>
					...
					<div className="flex flex-wrap gap-4 justify-center">
						<Button variant="secondary" className="flex items-center gap-2" onClick={handleDownloadReport}>
							<svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
								<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
							</svg>
							Download Report (PDF)
						</Button>
						<Button variant="secondary" className="flex items-center gap-2" onClick={handleCreateEstimate}>
							<svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
								<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
							</svg>
							Create Estimate
						</Button>
						<Button variant="secondary" className="flex items-center gap-2" onClick={handleGenerateSupplement}>
							<svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
