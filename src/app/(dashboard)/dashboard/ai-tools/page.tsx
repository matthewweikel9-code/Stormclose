...
const tabs: { id: ActiveTab; label: string; icon: React.ElementType; badge?: string; badgeColor?: string }[] = [
  { id: 'objection', label: 'Objection Handler', icon: MessageSquare, badge: 'Pro', badgeColor: 'bg-storm-purple/20 text-storm-glow' },
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="storm-card p-6">
        <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
          <MessageSquare className="h-5 w-5 text-storm-glow" />
          Generate Response
        </h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-storm-border bg-storm-z0 px-4 py-2.5 text-sm text-white placeholder-slate-500 outline-none focus:border-storm-purple"
            />
          </div>
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full rounded-lg border border-storm-border bg-storm-z0 px-4 py-2.5 text-sm text-white placeholder-slate-500 outline-none focus:border-storm-purple resize-none"
              required
            />
          </div>
          <div>
              ...
              {(['consultative', 'confident', 'empathetic'] as const).map((tone) => (
                <button
                  ...
                  className={`rounded-lg border px-3 py-2 text-xs font-medium capitalize transition-all ${
                    form.tone === tone
                      ? 'border-storm-purple bg-storm-purple/10 text-white'
                      : 'border-storm-border bg-storm-z0 text-slate-400 hover:border-storm-border-light'
                  }`}
                >
                  {tone}
                </button>
        ...
        <div className="mt-6 border-t border-storm-border pt-4">
          ...
          <div className="space-y-1">
            {[
              ...
              <button
                ...
                className="w-full text-left rounded-lg px-3 py-2 text-xs text-slate-400 hover:bg-storm-z2 hover:text-white transition-colors"
              >
                &ldquo;{obj}&rdquo;
              </button>
            ))}
      ...
      <div className="storm-card p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          {result && (
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-storm-border px-3 py-1.5 text-xs text-slate-400 hover:bg-storm-z2 hover:text-white transition-colors"
            >
        ...
        {result ? (
          <div className="prose prose-invert max-w-none">
            <div className="rounded-xl bg-storm-z0 border border-storm-border p-4 text-sm text-slate-300 whitespace-pre-wrap leading-relaxed">
              {result}
            </div>
          </div>
        ) : (
...
function NegotiationPanel() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="storm-card p-6">
        <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
          <Shield className="h-5 w-5 text-storm-glow" />
          Negotiation Setup
        </h3>
        <div className="space-y-4">
          <div>
            ...
            <select
              ...
              className="w-full rounded-lg border border-storm-border bg-storm-z0 px-4 py-2.5 text-sm text-white outline-none focus:border-storm-purple"
            >
      ...
      {/* Chat */}
      <div className="lg:col-span-2 storm-card flex flex-col h-[600px]">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] rounded-xl px-4 py-3 ${
                msg.role === 'user'
                  ? 'bg-storm-purple/20 text-white'
                  : 'bg-storm-z0 border border-storm-border text-slate-300'
              }`}>
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="rounded-xl bg-storm-z0 border border-storm-border px-4 py-3">
                <Loader2 className="h-4 w-4 animate-spin text-storm-glow" />
              </div>
            </div>
          )}
        ...
        {/* Input */}
        <div className="p-4 border-t border-storm-border">
          {error && (
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-lg border border-storm-border bg-storm-z0 px-4 py-2.5 text-sm text-white placeholder-slate-500 outline-none focus:border-storm-purple"
            />
            <button
              onClick={() => handleSubmit()}
              disabled={loading || !situation.trim() || !state}
              className="flex items-center gap-2 rounded-lg bg-storm-purple px-4 py-2.5 text-sm font-semibold text-white hover:bg-storm-purple-hover disabled:opacity-50 transition-all"
            >
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-4">
        <div className="storm-card p-6">
          ...
          {/* OCR Upload */}
          <div className="mb-4 rounded-lg border-2 border-dashed border-storm-border p-4 text-center hover:border-storm-purple/50 transition-colors cursor-pointer"
      ...
      <div className="storm-card p-6">
        ...
        {result ? (
          <div className="rounded-xl bg-storm-z0 border border-storm-border p-4 text-sm text-slate-300 whitespace-pre-wrap leading-relaxed max-h-[600px] overflow-y-auto">
            {result}
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-16 text-center">
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="storm-card p-4">
        <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
          <Building2 className="h-5 w-5 text-storm-glow" />
          Insurance Carriers
        </h3>
        {carriers.length === 0 ? (
      ...
      <div className="lg:col-span-2 storm-card p-6">
        {loadingDetails ? (
          <div className="flex items-center justify-center py-20"><Loader2 className="h-8 w-8 animate-spin text-storm-purple" /></div>
        ) : selectedCarrier ? (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="rounded-xl bg-storm-z0 border border-storm-border p-3 text-center">
                <p className="text-2xl font-bold text-storm-glow">{selectedCarrier.supplementSuccessRate}%</p>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {selectedCarrier.commonDenials.map((denial, idx) => (
                    <div key={idx} className="flex items-center gap-2 rounded-xl bg-storm-z0 border border-storm-border px-3 py-2 text-sm text-slate-300">
              ...
              <div>
                ...
                <div className="space-y-1">
                  {selectedCarrier.negotiationTips.map((tip, idx) => (
                    <div key={idx} className="flex items-start gap-2 rounded-xl bg-storm-z0 border border-storm-border px-3 py-2 text-sm text-slate-300">
            ...
            {selectedCarrier.preferredDocumentation?.length > 0 && (
              <div>
                <h4 className="text-sm font-semibold text-white mb-2 flex items-center gap-2">
                  <FileText className="h-4 w-4 text-storm-glow" /> Preferred Documentation
                </h4>
                <div className="flex flex-wrap gap-2">
                  {selectedCarrier.preferredDocumentation.map((doc, idx) => (
                    <span key={idx} className="rounded-full bg-storm-purple/10 border border-storm-purple/20 px-3 py-1 text-xs text-storm-glow">
                      {doc}
                    </span>
                  ))}
                </div>
