	...
	try {
		...
		for (const presence of presenceRecords) {
			...
			if (mission) {
				...
				const completed = detail.stops.filter((s) => s.status !== "pending");
				stopsRemaining = detail.stops.filter((s) => s.status === "pending").length;
				recentOutcomes = completed.slice(-10).map((s) => s.status) as RepState["recentOutcomes"];

				// Calculate nearest stop distance
				const pendingStops = detail.stops.filter((s) => s.status === "pending");
