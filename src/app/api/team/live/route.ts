...
export async function GET(request: NextRequest) {
	try {
		...
		for (const presence of presenceRecords) {
			...
			const completedStops = stops ? stops.stops.filter((s) => s.status !== "pending").length : 0;
			...
			repMap.set(presence.userId, {
				...
				lat: presence.lat,
				lng: presence.lng,
				accuracyMeters: presence.accuracy,
		...
		for (const mission of missions) {
			...
			const completedStops = detail.stops.filter((s) => s.status !== "pending").length;
