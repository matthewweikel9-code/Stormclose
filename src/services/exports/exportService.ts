import { createJobNimbusClient, type JNContact } from "@/lib/jobnimbus";
import {
	createExport,
	getExportById,
	updateExportStatus,
...
export async function sendToJobNimbus(payload: JobNimbusPayload): Promise<{
	...
	const jnContact: JNContact = {
		...payload.contact,
		mobile_phone: payload.contact.mobile_phone ?? undefined,
		email: payload.contact.email ?? undefined,
	};
	const contactResult = await client.createContact(jnContact);
